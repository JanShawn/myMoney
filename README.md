...
- 獨立 JSON 用來手動建立完整備份與還原；介面會提示目前資料是否比最近一次 JSON 備份更新。匯入會做欄位 allowlist、大小／筆數上限與危險鍵過濾。
- 可選擇連結一份固定同步 JSON，手動讀寫 Google Drive 電腦版或其他同步資料夾；未連結時不影響本機保存與其他功能。
...
2. 如有台灣上市／上櫃股票或 ETF，到「投資持倉」輸入代號與數量，Yahoo Finance 的商品名稱和最新行情價會自動帶入。
3. （選用）到「收支規劃」建立薪資、房租等週期項目，查看月均與指定月份現金流。
4. 到「資產盤點」確認帳戶、投資、加權指數和 240MA 後保存快照。
5. 定期到「設定」下載附日期時間的 JSON 備份；需要 Excel 時再匯出。

## 範例資料

想先查看完整介面時，可以下載 [`examples/myMoney-demo-data.json`](./examples/myMoney-demo-data.json)，再到「設定」選擇「從 JSON 備份還原」。這份檔案包含虛構帳戶、持倉、現金驗算、週期收支與 6 個月盤點紀錄，不含任何真實個人資料或即時市場價格。
...
- **獨立 JSON 備份**：用於資料救援的時間點副本；每次下載都建立新檔，不與日常同步檔案共用。
- **同步檔案（選用）**：在 Chrome／Edge 連結固定 JSON。建議開始修改前先載入同步檔案最新版；完成修改後，透過儲存視窗選取同一個 JSON 並確認取代，程式會在寫入後重新讀取驗證。顯示「內容已同步」時，不必另外下載 JSON 備份。檔案位於 Google Drive 電腦版資料夾時，雲端傳輸由 Google Drive 負責，myMoney 不會登入 Google；仍需等同步軟體完成上傳。
- **Excel**：只匯出每日快照，不保存帳戶結構，也不作為復原格式。
- **外幣匯率**：成功取得後會連同更新時間保存在 JSON／IndexedDB，離線時保留上次匯率。

下載 JSON 備份時會開啟系統儲存視窗，檔名自動包含本機日期時間，例如 `myMoney-backup-2026-09-18-1730.json`，避免不同時間的備份互相覆蓋。
...
- 固定同步檔案使用 File System Access API，目前以最新版 Chrome／Edge 桌面版為主；同步 JSON 是明文，應放在私人且未共享的資料夾。
- 加權指數與 240MA 由證交所 FMTQIK 每日資料產生；瀏覽器無法跨站時使用建置時的官方快取。
- USD、JPY 使用 ExchangeRate-API 的每日參考匯率；不是銀行現鈔或即期實際成交價。
- 上市／上櫃商品名稱與行情價統一由 Yahoo Finance 提供；交易所商品目錄只用來判斷上市或上櫃代號。
