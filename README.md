...
- 獨立 JSON 用來手動建立完整備份與還原；介面會提示目前資料是否比最近一次備份更新。匯入會做欄位 allowlist、大小／筆數上限與危險鍵過濾。
...
5. 定期到「設定」下載附日期時間的獨立完整備份；需要 Excel 時再匯出。

## 範例資料

想先查看完整介面時，可以下載 [`examples/myMoney-demo-data.json`](./examples/myMoney-demo-data.json)，再到「設定」選擇「從備份檔還原」。這份檔案包含虛構帳戶、持倉、現金驗算、週期收支與 6 個月盤點紀錄，不含任何真實個人資料或即時市場價格。
...
- **同步檔案（選用）**：在 Chrome／Edge 連結固定 JSON。建議開始修改前先載入同步檔案最新版，修改完成後再將目前資料存回；寫入前會重新檢查版本，遠端較新或兩邊都改過時會停止並提示，避免覆蓋新資料。檔案位於 Google Drive 電腦版資料夾時，雲端傳輸由 Google Drive 負責，myMoney 不會登入 Google。
- **Excel**：只匯出每日快照，不保存帳戶結構，也不作為復原格式。
- **外幣匯率**：成功取得後會連同更新時間保存在 JSON／IndexedDB，離線時保留上次匯率。

下載獨立備份時會開啟系統儲存視窗，檔名自動包含本機日期時間，例如 `myMoney-backup-2026-09-18-1730.json`，避免不同時間的備份互相覆蓋。
