...
const NUMBER_KEYS = new Set([
  'totalAssets', 'totalLiabilities', 'netWorth', 'availableCash', 'reservedCash', 'totalStocks', 'stockRatio',
...
export async function exportSnapshotsToExcel(snapshots) {
  ...
  for (let row = 2; row <= sheet.rowCount; row += 1) {
    for (const col of [3, 4, 5, 6, 7, 8, 10, 12, 13, 14, 15]) sheet.getRow(row).getCell(col).numFmt = '#,##0.00'
    for (const col of [9, 11]) sheet.getRow(row).getCell(col).numFmt = '0.00%'
  }
